...
public class ExampleFluidBERBlockEntity extends BlockEntity implements TickableBlockEntity, MenuProvider {
    private static final Component TITLE =
            Component.translatable("container." + TutorialMod.MODID + ".example_fluid_ber_block");

    private final ItemStackHandler inventory = new ItemStackHandler(1) {
        @Override
        protected void onContentsChanged(int slot) {
            super.onContentsChanged(slot);
            ExampleFluidBERBlockEntity.this.sendUpdate();
        }
    };
    ...
    public ExampleFluidBERBlockEntity(BlockPos pPos, BlockState pBlockState) {
        super(BlockEntityInit.EXAMPLE_FLUID_BER_BLOCK_ENTITY.get(), pPos, pBlockState);
    }
    ...
    public void tick() {
        ...
        fluidHandler.ifPresent(iFluidHandlerItem -> {
            if(!this.fluidTank.getFluid().isFluidEqual(iFluidHandlerItem.getFluidInTank(0)) && !this.fluidTank.isEmpty())
                return;
    ...
    public AbstractContainerMenu createMenu(int pContainerId, @NotNull Inventory pPlayerInventory, @NotNull Player pPlayer) {
        return new ExampleFluidBERMenu(pContainerId, pPlayerInventory, this);
    }
